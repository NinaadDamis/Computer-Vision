...
##### your code here #####
##########################
print("Size train ", train_x.shape)
...
saved = np.load('q3_weights.pickle', allow_pickle = True)
# Run Forward inference
a = forward(test_x,saved,name='layer1',activation=sigmoid)
out = forward(a,saved,name='output',activation=softmax)

# Get labels
preds = np.argmax(out,axis = 1) # Collapse to one column
gt = np.argmax(test_y, axis = 1)

for i in range(preds.shape[0]):

    confusion_matrix[gt[i],preds[i]] +=1

print("CF shape ", confusion_matrix.shape)
